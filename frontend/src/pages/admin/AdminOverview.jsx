...
function StatCard({ label, value, sub }) {
  return (
    <div className="bg-white p-5 rounded-2xl border border-[#1C2541]/10 shadow-sm">
      ...
      <p className="text-3xl font-bold text-[#1C2541]">{value}</p>
...
export default function AdminOverview() {
  ...
  return (
    <div>
      ...
      <h2 className="text-3xl font-bold text-[#1C2541] mb-8">Overview</h2>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        <StatCard label="Total Registered" value={participantTotal} />
        <StatCard label="Programs" value={programs.length} />
        <StatCard label="Active Now" value={ongoing.length} sub={ongoing.map(p=>p.name).join(', ') || 'N/A'} />
        <StatCard label="Upcoming" value={upcoming.length} sub={upcoming.map(p=>p.name).join(', ') || 'N/A'} />
      </div>

      {ongoing.length > 0 && (
        <div className="mb-10">
          <h3 className="font-bold text-[#1C2541] mb-4">Active Retreats</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {CATEGORIES.map(cat => (
          <div key={cat} className="bg-white p-5 rounded-2xl border border-[#1C2541]/10 shadow-sm">
            <h4 className="font-bold text-[#1C2541] mb-3">{cat}</h4>
