...
async function generateRetreatPDF(retreat, qrSvg) {
  ...
  doc.roundedRect(20, 55, 170, 32, 3, 3, 'F');
  doc.setDrawColor(...GOLD);
  doc.setLineWidth(0.6);
  doc.roundedRect(20, 55, 170, 32, 3, 3, 'S');
  doc.setFont('times', 'bold');
  doc.setFontSize(11);
...
function ManageSessions({ program, onBack }) {
  ...
  const loadDays = (signal) => apiService.getDays(program.id, signal ? { signal } : {}).then(r => {
    const data = r.data.results ?? r.data;
    setDays(data);
    if (!selDay && data.length) setSelDay(data[0]);
  }).catch(err => {
    if (!isCanceled(err)) {
      console.error(err);
  ...
  const loadSessions = (day, signal) => apiService.getSessions(program.id, day.id, signal ? { signal } : {})
  ...
  useEffect(() => {
    if (!selDay) return;
    const controller = new AbortController();
    loadSessions(selDay, controller.signal);
    return () => controller.abort();
  }, [selDay]);
  ...
  const addSession = async () => {
    if (!newSes.title || !newSes.start_time) {
...
export default function AdminPrograms() {
  ...
  const load = async (signal) => {
    try {
      const r = await apiService.getPrograms(signal ? { signal } : {});
      setPrograms(r.data.results ?? r.data);
    } catch (err) {
      if (!isCanceled(err)) toast.error('Failed to load programs.');
    }
  };
  ...
  const handleAdd = async () => {
    if (!newProgram.name || !newProgram.start_date || !newProgram.end_date) {
      toast.warning('Name and dates are required.');
      return;
    }
    try {
  ...
  const statusBadge = (s) => {
    const map = {
      ...
      grace: 'bg-[#D4A857]/20 text-[#6E2C3A]',
